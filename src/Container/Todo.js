...
/*title:obj.title,//{this.state.title}
  ...
  renderMessageList = () => {
    const messageViews = this.state.messages.map((item,i) => {
      return <MessageItemView idx={i} item={item} duoxuan={this.state.duoxuan} select={this.state.select} /*onClick={this.onItemClick}*/ onClick={this.onfuxuanClick} onduoxuanClick={this.onduoxuanClick} />
    });
    return messageViews;
  }
  Footer=()=>{
